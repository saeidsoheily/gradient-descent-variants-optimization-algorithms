...
Adagrad [DHS11] is an algorithm for gradient-based optimization that adapts the different learning rate to the parameters, performing larger updates for infrequent parameters and smaller updates for the frequent ones. Therefore, it is well-suited to deal with the sparse data. As a reminder, the SGD update for every parameter θi at each time step  t  then becomes:  
...
Gt ∈ ℝd×d is a diagonal matrix where each diagonal element i,i is the sum of the squares of the gradients w.r.t. θi up to time step t  and ϵ is a smoothing term that avoids division by zero (usually on the order of 1e−8).
...
J is the objective function, γ is the momentum decay term and η is the step size.
